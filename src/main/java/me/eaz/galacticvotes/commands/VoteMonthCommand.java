...
public class VoteMonthCommand implements CommandExecutor {

    private final GalacticVotes plugin;

    public VoteMonthCommand(GalacticVotes plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        sender.sendMessage(MessageUtil.color(plugin.getConfig().getString("messages.votemonth-header", "&6&m---&r &6&lTop Monthly Voters &6&m---")));

        List<Map.Entry<UUID, Integer>> leaderboard = plugin.getVoteManager().getMonthlyLeaderboard(size);
